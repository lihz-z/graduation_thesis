...
本章围绕雨天环境下车道线检测面临的雨滴遮挡、水膜反射和对比度下降等问题，以 CLRNet 为基线模型，给出了抗干扰模块的设计思路与实现说明。通过对基线模型结构的分析，归纳其在雨天场景下的三类典型失效模式；结合频域特性分析，提出了“频域门控 + 方向注意力”的轻量化改进思路；进一步结合当前代码实现，说明了模块的组成方式及其与 CLRNet 的融合位置，并给出了配套的训练目标和训练策略。

本章提出的算法框架为后续实验验证与部署分析提供了方法基础。围绕该模块的整体性能、分支贡献、参数灵敏度及部署适配性，将在第五章展开分析。
